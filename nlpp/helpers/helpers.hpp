...
template <class Base>
template <class Derived_, class Derived, std::enable_if_t<std::is_base_of<Base, Derived>::value, int>>
PolyClass<Base>::PolyClass (Derived_&& derived) : impl(std::make_unique<Derived>(std::forward<Derived>(derived)))
{
}
...
template <class Base>
template <class Derived_, class Derived>
PolyClass<Base>& PolyClass<Base>::operator= (Derived_&& derived)
{
